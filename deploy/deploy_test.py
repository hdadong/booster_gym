...
class Controller:
    ...
    def run(self):
        ...
        if self.step != 0:
            self.data_buffers[0]['state'].append(np.array(self.policy.obs, copy=True))
            self.data_buffers[0]['wm_state'].append(np.array(self.policy.wm_obs, copy=True))
            self.data_buffers[0]['priv_state'].append(np.array(self.policy.priv_obs, copy=True))
            self.data_buffers[0]['actions'].append(np.array(self.policy.actions, copy=True))
            self.data_buffers[0]['torques'].append(np.array(self.torques[11:], copy=True))
